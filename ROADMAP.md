...
- [x] `media_assets` ingestion path: every image requires license, creator, source URL, credit line, alt text before display. The published set now seeds public-domain photos from `apps/web/public/media/`. Vercel Blob remains optional if we outgrow the repo.
...
- [x] Media sourcing pass for published stories: public-domain government photography, National Archives, one CC0 NOAA photograph, a 1968 Warner/Reprise Hendrix publicity still treated as U.S. public domain because it was published without a copyright notice, and an 1897 Seattle newspaper page — license recorded per asset; no image without cleared rights. Repeat this pass as new stories are added.
